...
const Graph = () => {
  ...
  const [option, setOption] = React.useState<string>("age");
  ...
  const bars = covidData[option].cleanData.map(
    (el: InitialResponseType, i: number) => {
      const height = Math.floor(
        range(
          covidData[option].min,
          covidData[option].max,
          50,
          mainRef.current?.clientHeight! - 100,
          el.covid_19_deaths as number
        )
      );
      return (
        <React.Fragment key={"fragment" + i}>
          <GraphBar
            data-testid={`graphBar${i}`}
            data={el}
            key={i}
            height={height}
            visible={visible}
  ...
  return (
    <LabelContainer>
      <GraphOptions
        dateUpdated={covidData[option].updated}
        options={option}
        handleOptions={handleOptions}
      />
      <LabelY>{YLabel}</LabelY>
      <GraphContainer ref={mainRef}>{bars}</GraphContainer>
